...
app.get('/concerto/:evento', function(req, res){
  if (cached = cacheGet(req.params.evento)){
    res.send(cached);
  }
  else{
...
app.get('/detail/:evento', function(req, res){
  if (cached = cacheGet(req.params.evento)){
    res.send(cached);
  }
  else{
    var con = new Ticketek(req.params);
    con.getDetail(function(result){ 
      cacheSave(req.params.evento, result, {days: 5});
      res.json(result);
    });
  }
});
